...
<style>
:root{
  --bg:#080d15;--panel:#111827;--panel2:#182438;--text:#f8fafc;--muted:#9ca3af;
  --cyan:#38bdf8;--ocean:#0284c7;--ocean2:#22d3ee;--pink:#38bdf8;
}
...
div[data-baseweb="tab-list"]{
  gap:8px!important;background:#0f1726!important;border:1px solid #263349!important;
  border-radius:14px!important;padding:7px!important;overflow:visible!important;
}
button[data-baseweb="tab"]{
  background:#151f31!important;border:1px solid #2b3950!important;border-radius:10px!important;
  padding:11px 13px!important;min-height:46px!important;color:#aeb8c7!important;
  font-weight:800!important;justify-content:center!important;white-space:normal!important;
  text-align:center!important;line-height:1.2!important;
}
button[data-baseweb="tab"][aria-selected="true"]{
  background:linear-gradient(90deg,#0284c7,#22d3ee)!important;border-color:#67e8f9!important;
  color:white!important;box-shadow:0 5px 16px rgba(2,132,199,.28)!important;
}
...
.st-key-api_menu_container button:hover{background:#111!important;border-color:#fff!important}
div[data-baseweb="popover"]{z-index:1000001!important;}
div[data-baseweb="popover"] [data-testid="stVerticalBlock"]{min-width:min(88vw,390px);}
...
@media(max-width:700px){
  ...
  div[data-baseweb="tab-list"]{
    ...
    width:100%!important;gap:7px!important;padding:7px!important;
  }
  button[data-baseweb="tab"]{
    width:100%!important;min-width:0!important;padding:10px 5px!important;
  ...
  .st-key-api_menu_container button{width:42px!important;height:38px!important;min-height:38px!important}
}
...
.block-container{
  width:100%!important;max-width:1180px!important;box-sizing:border-box!important;
  overflow-x:hidden!important;
}
.st-key-srt_actions{
  width:100%!important;max-width:100%!important;overflow:hidden!important;
  margin:8px 0 0!important;padding:0!important;border-radius:13px!important;
  background:#0ea5e9!important;box-sizing:border-box!important;
}
.st-key-srt_actions div[data-testid="stHorizontalBlock"]{
  display:grid!important;grid-template-columns:minmax(0,1fr) minmax(0,1fr)!important;
  column-gap:1px!important;row-gap:0!important;width:100%!important;
...
.st-key-srt_actions button{
  width:100%!important;min-width:0!important;max-width:100%!important;
  min-height:52px!important;height:52px!important;margin:0!important;
  padding:5px 3px!important;border:0!important;border-radius:0!important;
  white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;
  line-height:1.05!important;font-size:clamp(10px,3vw,16px)!important;
  box-sizing:border-box!important;background:linear-gradient(90deg,#0284c7,#22d3ee)!important;
}
...
@media(max-width:430px){
  ...
  .st-key-srt_actions{width:100%!important;max-width:100%!important;min-width:0!important;}
  .st-key-srt_actions button{height:48px!important;min-height:48px!important;
...
.st-key-public_login_wrap{
  width:min(100%,760px)!important;margin:0 auto!important;
}
.st-key-public_login_wrap [data-testid="stMarkdownContainer"] h3{
  color:#ffc400!important;font-size:clamp(28px,7vw,43px)!important;
  font-weight:950!important;margin:18px 0 12px!important;line-height:1.2!important;
}
.st-key-customer_login_box{
  border:1px solid #1f2937!important;border-radius:16px!important;
  padding:18px 20px 16px!important;background:rgba(7,12,20,.42)!important;
}
.st-key-customer_login_box label,
.st-key-customer_login_box label p{
  color:#ffb000!important;font-weight:850!important;font-size:17px!important;
}
.st-key-customer_login_box input{
  min-height:58px!important;border-radius:12px!important;background:#f3f4f6!important;
  color:#20242e!important;border:1px solid #d1d5db!important;font-size:18px!important;
}
.st-key-customer_login_box input::placeholder{color:#8b8f99!important;opacity:1!important;}
.st-key-customer_login_box [data-testid="stFormSubmitButton"] button{
  min-height:58px!important;margin-top:10px!important;border-radius:12px!important;
  border:1px solid #ffd84d!important;
  background:linear-gradient(90deg,#ffab00 0%,#ffd600 100%)!important;
  color:#ffffff!important;font-weight:950!important;font-size:18px!important;
...
.social-split{
  ...
  gap:1px;padding:7px;margin:12px 0 0;border:2px solid #f5b400;
  border-radius:16px;overflow:hidden;background:#f5b400;box-sizing:border-box;
}
.social-split a{
  min-width:0;min-height:76px;display:flex;align-items:center;justify-content:center;
  gap:12px;color:#fff!important;text-decoration:none!important;
  font-size:clamp(16px,4vw,25px);font-weight:900;line-height:1;
  box-sizing:border-box;-webkit-tap-highlight-color:transparent;
}
.social-split a:first-child{
  border-radius:10px 0 0 10px;background:linear-gradient(135deg,#1265e8,#2f8df5);
}
.social-split a:last-child{
  border-radius:0 10px 10px 0;background:linear-gradient(135deg,#1aaee8,#36c9ef);
}
...
.login-help{
  margin:20px 2px 0;color:#a7adb7;font-size:clamp(15px,3.8vw,20px);line-height:1.65;
}
...
- NEVER write in Thai script. NEVER write in Chinese characters. NEVER write in English. Use ONLY Khmer Unicode script (U+1780–U+17FF).
...
- CRITICAL: Output ONLY in Khmer script (អក្សរខ្មែរ). Never output Thai, Chinese, Vietnamese, or English text.
"""
...
def parse_json_array(raw_text):
    ...
    try:
        ...
        raise ValueError(f"AI JSON ខូច៖ {e}")
...
def translation_needs_repair(cue, item):
    ...
    if not dialogue or contains_cjk(dialogue) or contains_thai(dialogue):
        return True
    if len(dialogue) > 3 and not has_khmer_script(dialogue):
        return True
...
def repair_translation_items(client, model_name, uploaded_video, cues, items):
    ...
    for _attempt in range(3):
        ...
        for offset in range(0, len(bad_ids), 12):
            ...
            prompt = TRANSLATE_PROMPT + "\nIMPORTANT: These cues failed before. Translate EVERY audible word, tiny response, negation, name, number, filler, and emotional reaction fully into natural Khmer. Never omit or summarize any element. Never copy Chinese or Thai characters. Use ONLY Khmer script (អក្សរខ្មែរ).\n\nCUES:\n" + payload
            ...
            for row in parse_json_array(response.text or ""):
                ...
                if dialogue and not contains_cjk(dialogue) and not contains_thai(dialogue):
...
6. Write ONLY in Khmer Unicode script (U+1780-U+17FF). Never output Thai script.
7. Keep each line concise enough for its timestamp, but preserve the full meaning.
8. Select one tag: M_ADULT, F_ADULT, M_OLD, F_OLD, BOY, GIRL,
   M_THINK, F_THINK, NARRATOR_M, NARRATOR_F.
9. JSON format: [{{"id":1,"tag":"M_ADULT","text":"..."}}]
...
def video_to_srt(video_path, api_keys, model, prepared_cues=None):
    """
    Unified v6.3 path:
    1. Try video-context translation first (best quality)
    2. Fall back to text-only translation (saves quota, no video upload needed)
    3. Auto-rotate API keys and models on any failure
    """
    ...
    for api_key_value in api_keys:
        ...
        # Try uploading video for context; if it fails, we still have text-only fallback
        uploaded_video = None
        try:
            uploaded_video = upload_for_context(client, video_path)
        except Exception as upload_err:
            pass  # Will fall back to text-only

        for model_name in _candidate_gemini_models(model):
            # Strategy 1: Video context (best quality)
            if uploaded_video is not None:
                try:
                    translated = translate_cues(client, model_name, uploaded_video, cues)
                    translated = repair_translation_items(
                        client, model_name, uploaded_video, cues, translated
                    )
            ...
            # Strategy 2: Text-only fallback (saves quota, works without video)
            try:
                translated = translate_cues_text_only(client, model_name, cues)
                translated = repair_translation_items(
                    client, model_name, None, cues, translated
                )
                result = build_srt(cues, translated)
                if result.strip() and "-->" in result:
                    return result
...
def analyze_inner_thoughts(srt_text, api_key, model_name, video_path=None):
    ...
    if video_path and Path(video_path).exists():
        try:
            context = upload_for_context(client, video_path)
        except Exception:
            pass
    updated = {}
    batch_size = 35
...
def create_mp3(srt_text, progress_callback=None):
    ...
    v3.1 fixes: better short-clip safeguards, Thai/Chinese detection, Khmer validation.
    """
    cues = parse_srt(srt_text)
    if not cues:
        raise ValueError('រកមិនឃើញ SRT និង timestamp ត្រឹមត្រូវទេ។')

    foreign_rows = [i + 1 for i, cue in enumerate(cues) if contains_cjk(cue['text']) or contains_thai(cue['text'])]
    if foreign_rows:
        raise ValueError(
            f'SRT នៅមានអក្សរបរទេស (ចិន/ថៃ) នៅបន្ទាត់៖ {foreign_rows[:20]}។ '
            'សូម Generate SRT ឡើងវិញ។'
        )

    no_khmer_rows = [i + 1 for i, cue in enumerate(cues) if len(cue['text']) > 3 and not has_khmer_script(cue['text'])]
    if no_khmer_rows:
        raise ValueError(
            f'SRT មិនមានអក្សរខ្មែរនៅបន្ទាត់៖ {no_khmer_rows[:20]}។ '
            'សូម Generate SRT ឡើងវិញ។'
        )
    ...
    with tempfile.TemporaryDirectory() as folder:
        ...
        for index, cue in enumerate(cues):
            ...
            # v3.1 safeguard: ensure trim is long enough for fades
            ...
            min_needed = fade_in + fade_out + 0.06
            if trim_seconds < min_needed:
                trim_seconds = min(min_needed, slot_seconds)
                fade_in = min(VOICE_FADE_IN_SECONDS, max(0.01, trim_seconds * 0.08))
                fade_out = min(VOICE_FADE_OUT_SECONDS, max(0.02, trim_seconds * 0.10))
...
# ---------------------------------------------------------------------------
# PRIVATE CUSTOMER LOGIN + HIDDEN OWNER LICENSE MANAGEMENT + PROJECTS
# ---------------------------------------------------------------------------
...
def initialize_license_database():
    with license_connection() as connection:
        ...
        # Projects table for customer workspace
        connection.execute(
            """
            CREATE TABLE IF NOT EXISTS projects (
                ...
                source_stem TEXT NOT NULL DEFAULT '',
                created_at TEXT NOT NULL,
        ...
        connection.execute("CREATE INDEX IF NOT EXISTS idx_projects_customer ON projects(customer_code)")
        connection.commit()
...
# PROJECT MANAGEMENT
# ---------------------------------------------------------------------------
def save_project(customer_code, project_name, srt_text, source_stem):
    if not customer_code or not project_name or not project_name.strip():
        return False
    now = _iso()
    with license_connection() as conn:
        existing = conn.execute(
            "SELECT id FROM projects WHERE customer_code=? AND project_name=?",
            (customer_code, project_name.strip())
        ).fetchone()
        if existing:
            conn.execute(
                "UPDATE projects SET srt_text=?, source_stem=?, updated_at=? WHERE id=?",
                (srt_text, source_stem, now, existing["id"])
            )
        else:
            conn.execute(
                "INSERT INTO projects (customer_code, project_name, srt_text, source_stem, created_at, updated_at) VALUES (?,?,?,?,?,?)",
                (customer_code, project_name.strip(), srt_text, source_stem, now, now)
            )
        conn.commit()
    return True


def load_projects(customer_code):
    with license_connection() as conn:
        return conn.execute(
            "SELECT * FROM projects WHERE customer_code=? ORDER BY updated_at DESC",
            (customer_code,)
        ).fetchall()


def delete_project(project_id, customer_code):
    with license_connection() as conn:
        conn.execute(
            "DELETE FROM projects WHERE id=? AND customer_code=?",
            (int(project_id), customer_code)
        )
        conn.commit()


def get_project_by_name(customer_code, project_name):
    with license_connection() as conn:
        return conn.execute(
            "SELECT * FROM projects WHERE customer_code=? AND project_name=?",
            (customer_code, project_name)
        ).fetchone()
...
# ---------------------------------------------------------------------------
customer_code = st.session_state.get("customer_code", "")
projects = load_projects(customer_code) if customer_code else []

with st.container():
    st.markdown('<div class="project-bar">', unsafe_allow_html=True)
    proj_col1, proj_col2, proj_col3, proj_col4 = st.columns([2.5, 1.2, 0.8, 0.8])
    ...
    with proj_col1:
        ...
        if selected_project != "🆕 New Project" and selected_project in project_map:
            proj = project_map[selected_project]
            if st.session_state.get("loaded_project_id") != proj["id"]:
                st.session_state.loaded_project_id = proj["id"]
                st.session_state.srt_text = proj["srt_text"]
                st.session_state.main_srt_editor = proj["srt_text"]
                st.session_state.source_video_stem = proj["source_stem"] or "khmer_story"
                st.session_state.audio_bytes = None
                st.session_state.pending_editor_update = proj["srt_text"]
                st.rerun()
        elif selected_project == "🆕 New Project":
            if st.session_state.get("loaded_project_id") is not None:
    ...
    with proj_col2:
        default_name = selected_project if selected_project != "🆕 New Project" else f"Project {_utcnow().strftime('%Y-%m-%d %H:%M')}"
        new_name = st.text_input(
            "ឈ្មោះ Project",
            value=st.session_state.get("current_project_name", default_name),
    ...
    with proj_col3:
        if st.button("💾 រក្សា", key="save_project_btn", use_container_width=True):
            if new_name.strip() and new_name.strip() != "🆕 New Project":
                save_project(
                    customer_code,
                    new_name.strip(),
                    st.session_state.get("srt_text", ""),
                    st.session_state.get("source_video_stem", "khmer_story")
                )
                st.success("✅ រក្សាទុករួច!")
                time.sleep(0.5)
                st.rerun()
            else:
                st.warning("សូមកំណត់ឈ្មោះ Project")

    with proj_col4:
        if selected_project != "🆕 New Project" and st.button("🗑️ លុប", key="delete_project_btn", use_container_width=True):
            proj_id = project_map[selected_project]["id"]
            delete_project(proj_id, customer_code)
            st.session_state.loaded_project_id = None
            st.session_state.srt_text = ""
            st.session_state.main_srt_editor = ""
            st.rerun()
...
st.caption("AI-KHEMRA-BRO v6.3-FIXED • Chinese Story Translation • Mobile-first")
