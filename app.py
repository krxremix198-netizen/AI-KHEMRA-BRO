...
'M_THINK':{'voice':PISITH,'rate':'-8%','pitch':'-4Hz','volume':'-8%'},
'F_THINK':{'voice':SREYMOM,'rate':'-8%','pitch':'-4Hz','volume':'-8%'},
...
real speaker pauses. Use one ellipsis (…) only for a meaningful hesitation or held
...
def clear_private_user_session(delete_saved_api=False):
    ...
    for state_key, default_value in {
        ...
        "audio_timing_warnings": [],
        "speech_tab_timing_warnings": [],
...
def character_voice_filters(tag):
    ...
    mapping = {
        ...
        'M_THINK': [
            'equalizer=f=180:t=q:w=1.0:g=0.6', 'equalizer=f=1200:t=q:w=1.0:g=-1.4',
            'equalizer=f=3200:t=q:w=1.0:g=-0.8', 'lowpass=f=6200:p=2', 'volume=0.72'
        ],
        'F_THINK': [
            'equalizer=f=220:t=q:w=1.0:g=0.4', 'equalizer=f=1200:t=q:w=1.0:g=-1.4',
            'equalizer=f=3400:t=q:w=1.0:g=-0.8', 'lowpass=f=6200:p=2', 'volume=0.72'
        ],
...
def create_mp3(srt_text, progress_callback=None, background_music_path=None, ducking_config=None, timing_warnings=None):
    """
    ...
    If ``timing_warnings`` is a list, this function appends one entry per
    cue whose Khmer speech did not fit inside its available slot even at
    MAX_TEMPO_SPEED, so the caller can tell the user exactly which cues were
    cut short instead of the audio silently drifting out of sync.
    """
    ...
    with tempfile.TemporaryDirectory() as folder:
        ...
        for index, cue in enumerate(cues):
            ...
            # Preserve the natural Edge-TTS rhythm. Only a very small tempo
            # adjustment is permitted; the translation prompt is responsible for
            # concise cue wording instead of forcing speech to rush.
            ...
            # Even at MAX_TEMPO_SPEED the Khmer voice can still be longer than
            # the time this cue has before the next one starts. When that
            # happens the clip is cut short below, so record which cue it was
            # instead of letting the mismatch pass silently.
            if timing_warnings is not None and rendered_seconds > slot_seconds + 0.06:
                cut_seconds = rendered_seconds - slot_seconds
        ...
        if st.button("💾 រក្សាទុក API Key", key="save_api_keys", use_container_width=True):
            ...
            if entered_keys:
                if save_private_api_keys(st.session_state.api_keys_manager):
                    ...
                    st.error("មិនអាចរក្សាទុក API Key ទៅគណនីបានទេ។ សោចាស់នៅតែរក្សាទុកដដែល។ សូមសាកល្បងម្តងទៀត។")
            else:
...
with tab_video:
                    ...
                    if workflow_mode == "🎙️ Khmer SRT + MP3 តែម្តង" and generated_srt != source_srt:
                        ...
                        try:
                            ...
                            generated_audio = create_mp3(
                                ...
                                timing_warnings=combined_timing_warnings,
                            )
                            st.session_state.audio_timing_warnings = combined_timing_warnings
                            notice = "✅ Khmer SRT និង MP3 បានបង្កើតរួចរាល់។"
                            if combined_timing_warnings:
                                cue_list = ", ".join(
                                    f"#{item['cue_number']}" for item in combined_timing_warnings[:10]
                                )
                                notice += (
                                    f" ⚠️ បន្ទាត់ {cue_list} មានអត្ថបទវែងជាងពេល cue ដូច្នេះសំឡេងត្រូវបានកាត់ខ្លីបន្តិច។"
                                )
    ...
    if not st.session_state.audio_bytes:
        ...
        if generate_clicked:
            if not st.session_state.srt_text.strip():
                ...
                try:
                    ...
                    manual_timing_warnings = []
                    st.session_state.audio_bytes = create_mp3(
                        st.session_state.srt_text,
                        progress_callback=update_audio_progress,
                        background_music_path=video_music_path,
                        ducking_config=video_ducking_config,
                        timing_warnings=manual_timing_warnings,
                    )
                    st.session_state.audio_timing_warnings = manual_timing_warnings
        ...
        if st.session_state.get("audio_timing_warnings"):
            cue_list = ", ".join(
                f"#{item['cue_number']} (កាត់ {item['cut_seconds']}s)"
                for item in st.session_state.audio_timing_warnings[:10]
            )
            st.warning(
                "⚠️ បន្ទាត់ខាងក្រោមមានអត្ថបទវែងជាងពេលវេលា cue ដូច្នេះសំឡេងត្រូវបានកាត់ខ្លីដើម្បីកុំឱ្យលើសពេល cue បន្ទាប់៖ "
                f"{cue_list}។ សូមកាត់ពាក្យ ឬបន្ថែមរយៈពេល cue ទាំងនេះ។"
            )
    ...
    def _clear_current_project():
        ...
        st.session_state.audio_timing_warnings = []
        st.session_state.speech_tab_timing_warnings = []
        ...
        if not speech_srt.strip():
            ...
            try:
                ...
                st.session_state.speech_tab_audio_bytes = create_mp3(
                    ...
                    timing_warnings=srt_timing_warnings,
                )
                st.session_state.speech_tab_timing_warnings = srt_timing_warnings
                waiting.empty()
                st.success("✅ បង្កើត MP3 រួចរាល់។")
            except Exception as exc:
                waiting.empty()
                st.error(f"❌ {exc}")
    if st.session_state.get("speech_tab_timing_warnings"):
        cue_list = ", ".join(
            f"#{item['cue_number']} (កាត់ {item['cut_seconds']}s)"
            for item in st.session_state.speech_tab_timing_warnings[:10]
        )
        st.warning(
            "⚠️ បន្ទាត់ខាងក្រោមមានអត្ថបទវែងជាងពេលវេលា cue ដូច្នេះសំឡេងត្រូវបានកាត់ខ្លីដើម្បីកុំឱ្យលើសពេល cue បន្ទាប់៖ "
            f"{cue_list}។ សូមកាត់ពាក្យ ឬបន្ថែមរយៈពេល cue ទាំងនេះ។"
...
st.caption("AI-KHEMRA-BRO v6.6.0 • Natural Audio + Music Ducking • Multilingual → Khmer • 4 Voice Tags • Mobile-first")
