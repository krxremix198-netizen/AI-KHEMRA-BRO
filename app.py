...
def create_mp3(srt_text, progress_callback=None, background_music_path=None, ducking_config=None):
    ...
    with tempfile.TemporaryDirectory() as folder:
        ...
        for index, cue in enumerate(cues):
            ...
            # Preserve natural Edge-TTS rhythm.  A line that cannot fit at the
            # small permitted tempo increase is rejected instead of being cut,
            # so the final MP3 can never trail beyond the intended SRT timing.
            ...
            if rendered_seconds > slot_seconds + 0.001:
                raise ValueError(
                    f"SRT cue {index + 1} វែងពេកសម្រាប់ timestamp ដើម។ "
                    "សូមកែសម្រួលអត្ថបទឱ្យខ្លីបែបធម្មជាតិ ឬបន្ថែមពេល cue; "
                    "App មិនកាត់សំឡេង ឬបង្ខំល្បឿនលើសកម្រិតទេ។"
                )
...
st.caption("AI-KHEMRA-BRO v6.7.0 • Ordered Khmer Translation • Strict SRT Timing • Natural Audio • Mobile-first")
