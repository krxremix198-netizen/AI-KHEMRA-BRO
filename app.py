...
/* Action identity: translation uses a brain pulse; voice generation shows the four canonical microphone roles. */
...
.voice-orchestra{
  display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:7px;margin:9px 0 12px;
}
.voice-orchestra .voice-mic{
  min-height:64px;display:flex;flex-direction:column;justify-content:center;align-items:center;
  gap:2px;border:1px solid rgba(34,211,238,.30);border-radius:13px;
  color:#dff8ff;background:linear-gradient(145deg,rgba(14,116,144,.24),rgba(15,23,42,.82));
  font-size:10px;font-weight:800;letter-spacing:.15px;
}
.voice-orchestra .voice-mic span{font-size:24px;line-height:1;display:block;animation:micFloat 2.2s ease-in-out infinite}
.voice-orchestra .voice-mic:nth-child(2) span{animation-delay:.20s}.voice-orchestra .voice-mic:nth-child(3) span{animation-delay:.40s}.voice-orchestra .voice-mic:nth-child(4) span{animation-delay:.60s}
.voice-orchestra .voice-mic:nth-child(3),.voice-orchestra .voice-mic:nth-child(4){opacity:.84}
@keyframes brainPulse{0%,100%{transform:scale(1)}50%{transform:scale(1.16) rotate(-4deg)}}
@keyframes micFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-4px)}}
@media (max-width:768px){.voice-orchestra{gap:5px}.voice-orchestra .voice-mic{min-height:56px;font-size:9px}.voice-orchestra .voice-mic span{font-size:21px}}
...
'M':{'voice':PISITH,'rate':'-1%','pitch':'+0Hz','volume':'+0%'},
'F':{'voice':SREYMOM,'rate':'-1%','pitch':'+0Hz','volume':'+0%'},
...
VOICE_FADE_OUT_SECONDS = 0.030
MIN_VOICE_GAP_MS = 0
MAX_TEMPO_SPEED = 1.10
# Short subtitle cues are common in fast dialogue. The app first requests a
# faster native Edge-TTS rendition, then uses a bounded final tempo fit for rare 1-second cues.
# This keeps complete words while the output remains inside the SRT window.
AUTO_FIT_MAX_TEMPO_SPEED = 1.60
AUTO_FIT_MAX_EDGE_RATE_PERCENT = 80
# Smooth only cue-to-cue loudness.  A long frame and small maximum gain avoid
# pumping individual Khmer syllables or flattening intended emotion.
FINAL_LEVELER_FILTER = 'dynaudnorm=f=800:g=5:p=0.92:m=1.25:n=1:c=1:b=1:o=0.75'
...
RULE 3 — STORY, MEANING, AND EMOTIONAL DEPTH: First understand the surrounding
story, the cause-and-effect of the scene, each speaker's relationship, and the emotion
behind the line. Preserve anger, humour, grief, romance, sarcasm, fear, wordplay, and
every plot-relevant detail. Rewrite naturally in Khmer so the same intended meaning and
emotion are heard. Never summarize, weaken, invent, or replace a specific event, clue,
promise, warning, condition, negation, name, number, or relationship only to make a
line shorter.

RULE 4 — LOCKED SRT TIMING: Preserve every ID, cue count, cue order, start timestamp,
and end timestamp exactly. Each Khmer line must be comfortable to speak inside its own
allotted cue. When a source line is long, use compact natural Khmer wording while
retaining EVERY audible meaning; never delete information, names, negation, numbers,
reactions, commands, or replies. Write one connected spoken idea per cue, with ordinary
punctuation only where a real speaker pauses. Use one ellipsis (…) only for a meaningful
hesitation or held thought, never repeatedly or as artificial padding. Do not make the Khmer voice rush,
drag, flatten its tone, or jump between artificial highs and lows. The generated voice starts exactly at each cue start and must finish at or before the cue end; do not borrow time from a following non-overlapping cue.
...
def build_multilingual_translation_prompt(cue_lines, source_language="Auto-detect", previous_context=""):
    return f"""
{KHMER_DUBBING_RULES}
...
try:
    configured_cookie_secret = str(os.getenv("COOKIE_SECRET") or st.secrets.get("COOKIE_SECRET", "")).strip()
except Exception:
    configured_cookie_secret = str(os.getenv("COOKIE_SECRET", "")).strip()

# Persisting credentials requires an owner-provided secret.  A hard-coded
# fallback would let anyone with this source decrypt customer browser cookies.
# PREVIOUS_COOKIE_SECRETS supports deliberate rotation of owner-managed secrets.
try:
    configured_previous_cookie_secrets = str(
        os.getenv("PREVIOUS_COOKIE_SECRETS") or st.secrets.get("PREVIOUS_COOKIE_SECRETS", "")
    ).strip()
except Exception:
    configured_previous_cookie_secrets = str(os.getenv("PREVIOUS_COOKIE_SECRETS", "")).strip()

COOKIE_SECRET_CONFIGURED = bool(configured_cookie_secret)
primary_cookie_secret = configured_cookie_secret
...
cipher_secrets = []
for secret in [primary_cookie_secret, *previous_cookie_secrets]:
...
api_cipher = api_ciphers[0] if api_ciphers else None
...
def load_secret_gemini_api_keys():
    """Read optional server-side fallback keys without placing them in the repository."""
    try:
        return _clean_api_keys(os.getenv("GEMINI_API_KEYS") or st.secrets.get("GEMINI_API_KEYS", ""))
    except Exception:
        return _clean_api_keys(os.getenv("GEMINI_API_KEYS", ""))
...
def encrypt_api_keys(api_keys_text):
    ...
    if not cleaned:
        return ""
    if api_cipher is None:
        raise RuntimeError("COOKIE_SECRET មិនទាន់កំណត់នៅលើ server ទេ។")
    return api_cipher.encrypt(cleaned.encode("utf-8")).decode("utf-8")


def decrypt_api_keys(cookie_value):
    """Read browser data encrypted with an owner-managed current or prior secret."""
    if not cookie_value or not api_ciphers:
        return ""
...
def save_upload(uploaded_file):
    ...
    if size <= 0 or size > 150 * 1024 * 1024:
        raise ValueError("វីដេអូត្រូវមានទំហំតិចជាង 150 MB និងមិនអាចទទេបានទេ។")
...
def cue_word_limit(start, end):
    """Khmer spoken-word budget that fits a natural voice inside its locked cue."""
    duration = max(0.35, float(end) - float(start))
    # This budget intentionally prefers concise, story-faithful Khmer over a
    # faster-than-natural voice.  The runtime never silently clips excess speech.
    return max(2, min(20, int(duration * 2.55 + 1.0)))
...
def translation_needs_repair(cue, item):
    ...
    if not dialogue or contains_non_khmer_script(dialogue):
        return True
    # Allow at most one token for Khmer tokenization quirks; longer lines are
    # repaired before TTS so the audio is not clipped to force it into the cue.
    return khmer_word_count(dialogue) > cue_word_limit(cue["start"], cue["end"]) + 1
...
def repair_translation_items(client, model_name, uploaded_video, cues, items):
    ...
    for _attempt in range(3):
        ...
        for offset in range(0, len(bad_ids), 12):
            ...
            payload = "\n".join(
                f'ID={cue["id"]} | TIME={seconds_to_srt(cue["start"])} --> {seconds_to_srt(cue["end"])} '
                f'| MAX_WORDS={cue_word_limit(cue["start"], cue["end"])} | SOURCE={cue["source"]}'
                for cue in group
            )
            prompt = TRANSLATE_PROMPT + "\nIMPORTANT: These cues failed before. Read the scene logic and translate EVERY audible word, tiny response, negation, name, number, filler, emotional reaction, command, clue, and condition into clean natural Khmer. Never omit, summarize, invent, or weaken any story element. Keep each original ID and locked TIME unchanged. Use compact Khmer wording only to fit MAX_WORDS, never by deleting meaning. Never copy foreign characters.\n\nCUES:\n" + payload
...
def _candidate_gemini_models(selected_model):
    ...
    # Keep the UI choice first, then use only current stable Google AI Studio
    # Gemini endpoints.  This allows a newly available model to be used without
    # changing any visible UI control or interrupting older user selections.
    ordered = [
...
def _translate_batch_text_only(client, model_name, batch, previous_context="", source_language="Auto-detect"):
    ...
    prompt = build_multilingual_translation_prompt(cue_lines, source_language, previous_context)
...
def ordered_translation_items(cues, translated):
    """Return exactly one accepted translation for every source cue in source order."""
    missing_ids = [cue["id"] for cue in cues if cue["id"] not in translated]
    if missing_ids:
        raise RuntimeError(
            "AI មិនបានបកប្រែតាមលំដាប់គ្រប់បន្ទាត់ទេ៖ "
            + ", ".join(map(str, missing_ids[:20]))
        )
    return {cue["id"]: translated[cue["id"]] for cue in cues}


def translate_cues_text_only(client, model_name, cues, source_language="Auto-detect"):
    """Fast, ordered multilingual-to-Khmer path with continuity and targeted repair."""
    translated = {}
    # Thirty cues retains the fast one-pass workflow while giving the model more
    # local story context and reducing skipped/reordered responses on long videos.
    batch_size = 30
    for offset in range(0, len(cues), batch_size):
        batch = cues[offset:offset + batch_size]
        context_rows = []
        for cue in cues[max(0, offset - 8):offset]:
        ...
        parsed = _translate_batch_text_only(
            client, model_name, batch, "\n".join(context_rows), source_language
        )
    ...
    return ordered_translation_items(cues, translated)



def video_to_srt(video_path, api_keys, model, prepared_cues=None, source_language="Auto-detect"):
    """
    Reliable v5.5 path:
    ...
    for api_key_value in api_keys:
        ...
        for model_name in _candidate_gemini_models(model):
            try:
                translated = translate_cues_text_only(client, model_name, cues, source_language)
...
def translate_srt_to_khmer(srt_text, api_keys, model, source_language="Auto-detect"):
...
def character_voice_filters(tag):
    ...
    mapping = {
        ...
        'M_THINK': [
            ...
            'lowpass=f=6800:p=2', 'volume=0.80'
        ],
        'F_THINK': [
...
def voice_tone_filters(tag):
    ...
    return [
        # Reduce rumble and sharp airy hiss without making Khmer consonants muddy.
        'highpass=f=85:p=2',
        'lowpass=f=7000:p=2',
        'equalizer=f=180:t=q:w=1.0:g=0.8',
        'equalizer=f=320:t=q:w=1.1:g=0.6',
        'equalizer=f=1100:t=q:w=1.2:g=0.4',
        'equalizer=f=2400:t=q:w=1.1:g=0.5',
        'equalizer=f=4300:t=q:w=1.0:g=-2.4',
        'equalizer=f=5800:t=q:w=0.9:g=-4.0',
        'equalizer=f=6800:t=q:w=0.8:g=-4.6',
        *character_voice_filters(canonical_tag),
        # Slow, low-ratio control prevents abrupt clip-level jumps, not emotion.
        'acompressor=threshold=-18dB:ratio=1.28:attack=60:release=400:makeup=1.00:knee=6',
    ]


def append_audio_master_filters(filters, voice_label, total_seconds=None, music_input_index=None, ducking_config=None):
    ...
    filters.append(
        ...
        + FINAL_LEVELER_FILTER + ','
          'acompressor=threshold=-16dB:ratio=1.18:attack=90:release=520:makeup=1.00:knee=7,'
          'alimiter=limit=0.92:attack=25:release=320,'
          'loudnorm=I=-17:TP=-2.0:LRA=7'
        + ending + '[out]'
    )
...
def create_mp3(srt_text, progress_callback=None, background_music_path=None, ducking_config=None):
    ...
    with tempfile.TemporaryDirectory() as folder:
        root = Path(folder)
        clips = [None] * len(cues)
        clip_durations = [0.0] * len(cues)
        total_cues = len(cues)

        if progress_callback:
            progress_callback(2, "កំពុងរៀបចំសំឡេងតួអង្គ…")

        def create_voice_clip(index, cue, rate_override=None):
            clip = root / f'clip_{index:04d}.mp3'
            profile = dict(VOICE_PROFILES.get(cue['tag'], VOICE_PROFILES['M']))
            if rate_override is not None:
                profile['rate'] = str(rate_override)
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = [executor.submit(create_voice_clip, index, cue) for index, cue in enumerate(cues)]
        ...
        # Re-synthesize only the occasional short cue at a faster native
        # Edge-TTS rate. This retains the whole sentence and avoids a visible
        # error for ordinary fast dialogue without adding time after the cue.
        auto_fitted_indexes = set()
        for index, cue in enumerate(cues):
            start_ms = max(0, int(cue['start']))
            end_ms = int(cue['end'])
            slot_seconds = max(0.01, (end_ms - start_ms) / 1000.0)
            required_speed = clip_durations[index] / slot_seconds
            if required_speed <= MAX_TEMPO_SPEED:
                continue
            native_factor = required_speed / AUTO_FIT_MAX_TEMPO_SPEED
            boost_percent = min(
                AUTO_FIT_MAX_EDGE_RATE_PERCENT,
                max(15, int((native_factor - 1.0) * 100.0) + 2),
            )
            if progress_callback:
                progress_callback(
                    88,
                    f"កំពុងសម្របសំឡេង cue {index + 1}/{total_cues} ឱ្យត្រូវ timestamp…",
                )
            _, clip, duration = create_voice_clip(index, cue, f"+{boost_percent}%")
            clips[index] = clip
            clip_durations[index] = duration
            auto_fitted_indexes.add(index)
        ...
        for index, cue in enumerate(cues):
            start_ms = max(0, int(cue['start']))
            cue_end_ms = int(cue['end'])

            # The SRT cue is the authoritative video window.  Original cue
            # overlaps are retained when characters really speak over each
            # other; no artificial milliseconds are added to any subtitle.
            protected_end_ms = cue_end_ms

            if protected_end_ms <= start_ms:
                raise ValueError(
                    f"SRT cue {index + 1} មិនមានពេលគ្រប់គ្រាន់សម្រាប់បង្កើតសំឡេង។ "
                    "សូមកែ timestamp ឬបែងអត្ថបទជា cue ផ្សេង។"
                )

            slot_seconds = (protected_end_ms - start_ms) / 1000.0
            audio_seconds = clip_durations[index]

            # Ordinary cues retain the gentle tempo cap. A short cue that was
            # re-synthesized at a native faster Edge-TTS rate may use a modest
            # additional fit, keeping its complete speech inside the SRT window.
            required_speed = audio_seconds / slot_seconds
            tempo_cap = AUTO_FIT_MAX_TEMPO_SPEED if index in auto_fitted_indexes else MAX_TEMPO_SPEED
            safe_speed = min(max(1.0, required_speed), tempo_cap)
            tempo = atempo_chain(safe_speed) if safe_speed > 1.001 else ''
            rendered_seconds = audio_seconds / safe_speed
            if rendered_seconds > slot_seconds + 0.001:
                raise ValueError(
                    f"SRT cue {index + 1} វែងខ្លាំងពេកសម្រាប់ timestamp ដើម។ "
                    "App បានសាកសម្របសំឡេងដោយស្វ័យប្រវត្តិរួចហើយ; "
                    "សូមបែង cue នេះជាពីរបន្ទាត់ ឬបន្ថែមពេល cue ដើម្បីរក្សាពាក្យគ្រប់គ្រាន់។"
                )
            trim_seconds = rendered_seconds

            # Short click-protection fades prevent staccato volume dips between cues.
            fade_in = min(VOICE_FADE_IN_SECONDS, max(0.008, trim_seconds * 0.035))
            fade_out = min(VOICE_FADE_OUT_SECONDS, max(0.012, trim_seconds * 0.050))
        ...
        # Keep the rendered MP3 aligned to the final SRT end; do not append
        # artificial audio time after the video dialogue has finished.
        total = max(0.35, final_end_ms / 1000.0)
        ...
        try:
            ...
            raise RuntimeError('ការលាយសំឡេងចំណាយពេលយូរពេក។ សូមកាត់ SRT ឬសាកម្ដងទៀត។') from exc
        except FileNotFoundError as exc:
            raise RuntimeError('ម៉ាស៊ីនមេមិនមាន FFmpeg សម្រាប់បង្កើត MP3 ទេ។') from exc
        if result.returncode != 0:
...
def create_single_voice_mp3(text, tag, background_music_path=None, ducking_config=None):
    ...
    with tempfile.TemporaryDirectory() as folder:
        ...
        try:
            ...
            raise RuntimeError('ការបង្កើតសំឡេងចំណាយពេលយូរពេក។ សូមកាត់អត្ថបទឱ្យខ្លី ហើយសាកម្ដងទៀត។') from exc
        except FileNotFoundError as exc:
            raise RuntimeError('ម៉ាស៊ីនមេមិនមាន FFmpeg សម្រាប់បង្កើត MP3 ទេ។') from exc
...
# ─────────────────────────────────────────────────────────────────────────────
# Set AI_KHEMRA_BRO_DATA_DIR to a mounted persistent directory in production.
# Falling back to the app directory is suitable only for local development.
_configured_data_dir = os.getenv("AI_KHEMRA_BRO_DATA_DIR", "").strip()
LICENSE_DB_PATH = (
    Path(_configured_data_dir).expanduser() / "licenses.db"
    if _configured_data_dir
    else Path(__file__).with_name("licenses.db")
)
PERSISTENT_LICENSE_STORAGE_CONFIGURED = bool(_configured_data_dir)
...
def admin_credentials_configured():
    """Require owner-managed administrator credentials; never ship a default password."""
...
def normalize_access_code(value):
    # Keep the documented underscore character and avoid silently truncating a
    # code that the validation layer says is valid.
        ...
        with st.expander(f"{row['customer_name']} • {row['access_code_display']} • {status}"):
            ...
            with action_right:
                if st.button("សម្អាត Session ចាស់", key=f"disconnect_{row['id']}", use_container_width=True):
                    ...
                    st.rerun()
            st.caption("Gemini API Key របស់អ្នកប្រើត្រូវរក្សាទុកតែក្នុង browser របស់គាត់។ Owner មិនអាចមើល ឬលុបវាពី server បានទេ។")
...
for state_key, default_value in {
    ...
    "translation_style": "🔴 Khmer Dubbing Pro",
...
if st.session_state.get("translation_style") not in ("🔴 Khmer Dubbing Pro", "⚪ Subtitle Sync"):
    st.session_state.translation_style = "🔴 Khmer Dubbing Pro"
...
with st.container(key="api_menu_container"):
    with st.popover("☰", help="API Key និងការកំណត់កម្មវិធី"):
        ...
        st.divider()
        st.selectbox("🌍 លទ្ធផល", ["Khmer (ខ្មែរ)"], key="target_language")
        st.selectbox(
        ...
        st.selectbox(
            "🚀 លំហូរការងារ",
            [
                "⚡ Khmer SRT ស្វ័យប្រវត្តិ",
                "🎙️ Khmer SRT + MP3 តែម្តង",
                "📝 Source SRT only",
            ],
            key="workflow_mode",
            help="ជ្រើសតែម្ដង ហើយកម្មវិធីនឹងដំណើរការតាមលំហូរដែលអ្នកជ្រើស ដោយមិនបង្ខំឱ្យធ្វើជំហានដោយដៃ។",
        )
        st.selectbox(
            "⚙️ ល្បឿនដំណើរការ",
            ["⚡ លឿន (ណែនាំ)", "🎚️ សំឡេងច្បាស់ (យឺតជាង)"],
            key="processing_mode",
            help="របៀបលឿនទាញសំឡេងដោយផ្ទាល់ទៅ Whisper។ របៀបច្បាស់បន្ថែមការសម្អាតសំឡេងសម្រាប់វីដេអូរំខាន។",
        )
        st.radio(
            "🎭 Translation Style",
            ["🔴 Khmer Dubbing Pro", "⚪ Subtitle Sync"],
        ...
        st.caption(
            "API Key ត្រូវបានអ៊ិនគ្រីប និងរក្សាទុកតែក្នុង browser នេះ។ "
        ...
        if not COOKIE_SECRET_CONFIGURED:
            st.warning("Server មិនទាន់កំណត់ COOKIE_SECRET ទេ ដូច្នេះ API Key មិនអាចរក្សាទុកក្នុង browser ដោយសុវត្ថិភាពបាន។")
        ...
        if st.button("💾 រក្សាទុក API Key", key="save_api_keys", use_container_width=True):
            ...
            if entered_keys:
                if save_private_api_keys(st.session_state.api_keys_manager):
                    st.session_state.api_saved_notice = True
                    st.rerun()
                else:
                    st.error("មិនអាចរក្សាទុក API Key ក្នុង browser បានទេ។ សូមពិនិត្យ COOKIE_SECRET របស់ server ហើយសាកម្ដងទៀត។")
            else:
...
with tab_video:
    ...
    uploaded_video = st.file_uploader(
        ...
        help="MP4 ត្រូវបានណែនាំ។ កម្មវិធីទាញសំឡេងពីវីដេអូដោយផ្ទាល់សម្រាប់ល្បឿនលឿន; ការបង្ហាប់វីដេអូមិនត្រូវបានធ្វើទេ លុះត្រាតែត្រូវការជួសជុលបញ្ហាពិសេស។",
        ...
        if size_mb > max_mb:
            ...
            if st.button("🚀 ចាប់ផ្ដើមលំហូរដែលបានជ្រើស", key="generate_srt", use_container_width=True):
                ...
                try:
                    ...
                    show_waiting("កំពុងរៀបចំសំឡេងពីវីដេអូ", "សូមរង់ចាំបន្តិច និងកុំបិទទំព័រនេះ។")
                        ...
                        try:
                            with ThreadPoolExecutor(max_workers=1) as executor:
                                future = executor.submit(
                                    video_to_srt, video_path, valid_api_keys, model, cues, source_language
                                )
                                while not future.done():
                                    elapsed = time.time() - started_at
    ...
    if st.button("បកប្រែ SRT → ខ្មែរ", key="translate_btn"):
        if not source_srt.strip():
            ...
            try:
                ...
                translated_srt = translate_srt_to_khmer(source_srt, valid_api_keys, model, source_language)
                waiting.empty()
