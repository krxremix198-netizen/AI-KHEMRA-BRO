...
# Build an owner-managed current cipher and validate round-trip encryption.
...
namespace["api_ciphers"] = [cipher_for(primary_secret)]
...
# Rotation accepts only owner-supplied predecessor secrets; the embedded legacy
# string must never be used as the default encryption key in production.
assert "PREVIOUS_COOKIE_SECRETS" in source_text
assert "for secret in [primary_cookie_secret, *previous_cookie_secrets]" in source_text
assert "configured_cookie_secret or LEGACY_COOKIE_SECRET" not in source_text
previous_secret = "unit-test-previous-secret"
namespace["api_ciphers"] = [cipher_for(primary_secret), cipher_for(previous_secret)]
